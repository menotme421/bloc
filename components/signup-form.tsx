...
export function SignupForm({
  ...
  const [state, formAction, isPending] = useActionState<AuthState, FormData>(
    isForgot ? resetPassword : isSignUp ? signUp : signIn,
    initialAuthState,
  );
  ...
  const showConfirmation = submitted && !!state.message && !state.error;
  const showError = submitted && !!state.error;
      ...
      <Card className="overflow-hidden rounded-md border border-border p-0 ring-0">
        <CardContent className="grid p-0 md:grid-cols-2">
          {showConfirmation ? (
            ...
            <form key={mode} action={handleSubmit} className="p-6 md:p-8">
              <FieldGroup>
